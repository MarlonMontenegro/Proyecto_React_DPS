...
export default function DashboardPage() {
  ...
  React.useEffect(() => {
    setProjects(safeGet(PROJECTS_KEY, []));
    setUsers(safeGet(USERS_KEY, []));
  }, []);

  // Persistencia
  const saveProjects = React.useCallback((next) => {
    setProjects(next);
    safeSet(PROJECTS_KEY, next);
  }, []);
  ...
  function onSubmitProject(e) {
    ...
    if (editingId) {
      ...
      const p = { id: Date.now(), ...base, members: [] };
  ...
  function endProject(pid) {
    ...
    saveProjects(next);
    if (editingId === pid) onCancelEdit();
  }

  // --------- Asignaciones (Gerente) ----------
  function assignProjectTo(email, projectId) {
  ...
  function unassignProjectFrom(email, projectId) {
    const pid = Number(projectId);
    const next = projects.map(p => {
      if (p.id !== pid) return p;
  ...
  function togglePhaseForUser(projectId, phaseId, email) {
    const next = projects.map(p => {
      if (p.id !== projectId) return p;
      const phases = (p.phases || []).map(f => {
        if (f.id !== phaseId) return f;
        const doneBy = new Set(f.doneBy || []);
        if (doneBy.has(email)) doneBy.delete(email); else doneBy.add(email);
        return { ...f, doneBy: Array.from(doneBy) };
      });
      return { ...p, phases };
    });
    saveProjects(next);
    ...
    <Protected>
      ...
      <RoleGate allow={["manager"]}>
        <section style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8, marginTop: 12 }}>
                ...
                {projects.map(p => (
                  <li key={p.id} className="row">
                    ...
                    <div className="row-actions">
                      ...
                      <button onClick={() => endProject(p.id)} className="danger">Terminar</button>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {/* --- Asignar tareas (proyectos a usuarios) --- */}
          {activeTab === "assign" && (
            <div className="card">
...
<style jsx>{`
  ...
  .grid input::placeholder, .grid textarea::placeholder { color: #9ca3af; } /* slate-400 */
  ...
  .card .grid select {
    ...
    border: 1px solid #374151; /* gris oscuro (slate-700) */
  }
